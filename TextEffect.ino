    ...
    if (charPosition == 0)
    {
      for (uint8_t i = 0; i < 8; i++)
      {
        for (uint8_t j = 0; j < 8; j++)
        {
          ...
          {
            cube[i][j] = BIT_CHECK(characters[textToDisplay[charCounter] - '0'][i], j);
          }
          else if (textToDisplay[charCounter] >= 'A' && textToDisplay[charCounter] <= 'Z')
          {
            cube[i][j] = BIT_CHECK(characters[textToDisplay[charCounter] - 'A' + 10][i], j);
          }
          else if (textToDisplay[charCounter] == '-')
          {
            cube[i][j] = BIT_CHECK(characters[36][i], j);
          }
        }
      }
    }
